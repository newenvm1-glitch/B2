...
LANGUAGES = {
    ...
    "fr": {
        "welcome": "Hi {user} bienvenue au Boinkers support bot! Ce bot aide con acceso au portefeuille, transactions, soldes, recoveries, récupération de compte, réclamer tokens et récompenses, remboursements et validations de compte. Veuillez choisir une option du menu pour continuer.",
    ...
    "ru": {
        "welcome": "Hi {user} добро пожаловать в Boinkers support bot! Этот бот помогает с доступом к кошельку, транзакциями, балансами, восстановлением, восстановлением аккаунта, получением токенов и наград, возвратами и проверкой аккаунта. Пожалуйста, выберите один из пунктов меню, чтобы продолжить.",
    ...
    "uk": {
        "welcome": "Hi {user} ласкаво просимо до Boinkers support bot! Цей бот допомагає з доступом до гаманця, транзакціями, балансами, відновленнями, відновленням облікового запису, отриманням токенів і винагород, поверненнями та перевірками облікового запису. Будь ласка, виберіть один із пунктів меню, щоб продовжити.",
    ...
    "fa": {
        "welcome": "Hi {user} خوش آمدید به Boinkers support bot! این بات به شما در دسترسی به کیف پول، تراکنش‌ها، موجودی‌ها، بازیابی‌ها، بازیابی حساب، درخواست توکن‌ها و جوایز، بازپرداخت‌ها و اعتبارسنجی حساب کمک می‌کند. لطفاً یک گزینه از منو را انتخاب کنید تا ادامه دهیم.",
    ...
    "ar": {
        "welcome": "Hi {user} مرحبًا بك في Boinkers support bot! يساعدك هذا البوت في الوصول إلى المحفظة، المعاملات، الأرصدة، الاسترداد، استرداد الحساب، المطالبة بالرموز والمكافآت، الاستردادات، والتحققات الحسابية. الرجاء اختيار خيار من القائمة للمتابعة.",
    ...
    "pt": {
        "welcome": "Hi {user} bem-vindo ao Boinkers support bot! Este bot ajuda com acesso à carteira, transações, saldos, recuperações, recuperação de conta, reivindicar tokens e recompensas, reembolsos e validações de conta. Por favor escolha uma opção do menu para prosseguir.",
    ...
    "id": {
        "welcome": "Hi {user} selamat datang di Boinkers support bot! Bot ini membantu dengan akses dompet, transaksi, saldo, recoveries, account recovery, klaim token dan reward, pengembalian dana, dan validasi akun. Silakan pilih opsi menu untuk melanjutkan.",
    ...
    "de": {
        "welcome": "Hi {user} willkommen beim Boinkers support bot! Dieser Bot hilft bei Wallet-Zugriff, Transaktionen, Kontoständen, Wiederherstellungen, Kontowiederherstellung, Token- und Belohnungsansprüchen, Rückerstattungen und Kontovalidierungen. Bitte wählen Sie eine Menüoption, um fortzufahren.",
    ...
    "nl": {
        "welcome": "Hi {user} welkom bij de Boinkers support bot! Deze bot helpt met wallet-toegang, transacties, saldi, herstel, account recovery, tokens en rewards claimen, terugbetalingen en accountvalidaties. Kies een optie uit het menu om door te gaan.",
    ...
    "hi": {
        "welcome": "Hi {user} Boinkers support bot में आपका स्वागत है! यह बोट वॉलेट एक्सेस, लेनदेन, बैलेंस, रिकवरी, अकाउंट रिकवरी, टोकन और रिवॉर्ड क्लेम, रिफंड और अकाउंट वेलिडेशन में मदद करता है। जारी रखने के लिए मेनू से एक विकल्प चुनें।",
    ...
    "tr": {
        "welcome": "Hi {user} Boinkers support bot'a hoş geldiniz! Bu bot cüzdan erişimi, işlemler, bakiye, kurtarmalar, hesap kurtarma, token ve ödül talepleri, iade ve hesap doğrulamaları konusunda yardımcı olur. Devam etmek için menüden bir seçenek seçin.",
    ...
    "zh": {
        "welcome": "Hi {user} 欢迎使用 Boinkers support bot! 此机器人可帮助钱包访问、交易、余额、恢复、账户恢复、认领代币与奖励、退款和账户验证。请选择菜单中的一项继续。",
    ...
    "cs": {
        "welcome": "Hi {user} vítejte u Boinkers support bot! Tento bot pomáhá s přístupem k peněžence, transakcemi, zůstatky, obnovami, obnovením účtu, nárokováním tokenů a odměn, refundacemi a validacemi účtu. Vyberte prosím možnost z nabídky pro pokračování.",
    ...
    "ur": {
        "welcome": "Hi {user} welcome to Boinkers support bot! This bot helps with wallet access, transactions, balances, recoveries, account recovery, claiming tokens and rewards, refunds, and account validations. Please choose one of the menu options to proceed.",
    ...
    "uz": {
        "welcome": "Hi {user} Boinkers support botga xush kelibsiz! Ushbu bot hamyonga kirish, tranzaksiyalar, balanslar, tiklash, hisobni tiklash, token va mukofotlarni talab qilish, qaytarishlar va hisob tekshiruvi kabi masalalarda yordam beradi. Davom etish uchun menyudan bir variant tanlang.",
    ...
    "it": {
        "welcome": "Hi {user} benvenuto al Boinkers support bot! Questo bot aiuta con l'accesso al wallet, transazioni, saldi, recuperi, recupero account, richiesta token e ricompense, rimborsi e validazioni account. Scegli un'opzione del menu per procedere.",
    ...
    "ja": {
        "welcome": "Hi {user} ようこそ Boinkers support bot へ！このボットはウォレットアクセス、トランザクション、残高、復旧、アカウント回復、トークンや報酬の請求、返金、アカウント検証を支援します。メニューから選択してください。",
    ...
    "ms": {
        "welcome": "Hi {user} selamat datang ke Boinkers support bot! Bot ini membantu dengan capaian wallet, transaksi, baki, pemulihan, pemulihan akaun, tuntutan token dan ganjaran, pulangan dan pengesahan akaun. Sila pilih pilihan menu untuk meneruskan.",
    ...
    "ro": {
        "welcome": "Hi {user} bine ați venit la Boinkers support bot! Acest bot ajută cu acces portofel, tranzacții, solduri, recuperări, recuperare cont, revendicare token-uri și recompense, rambursări și validări cont. Vă rugăm să alegeți o opțiune din meniu pentru a continua.",
    ...
    "sk": {
        "welcome": "Hi {user} vítajte pri Boinkers support bot! Tento bot pomáha s prístupom k peňaženke, transakciami, zostatkami, obnovami, obnovením účtu, tokenmi a odmenami, refundáciami a overením účtu. Vyberte možnosť v ponuke pre pokračovanie.",
    ...
    "th": {
        "welcome": "Hi {user} ยินดีต้อนรับสู่ Boinkers support bot! บอทนี้ช่วยเรื่องการเข้าถึงกระเป๋าเงิน, ธุรกรรม, ยอดคงเหลือ, การกู้คืน, การกู้คืนบัญชี, การเคลมโทเค็นและรางวัล, การคืนเงิน และการยืนยันบัญชี กรุณาเลือกตัวเลือกจากเมนูเพื่อดำเนินการต่อ",
    ...
    "vi": {
        "welcome": "Hi {user} chào mừng đến với Boinkers support bot! Bot này giúp truy cập ví, giao dịch, số dư, khôi phục, khôi phục tài khoản, yêu cầu token và phần thưởng, hoàn tiền và xác thực tài khoản. Vui lòng chọn một tùy chọn để tiếp tục.",
    ...
    "pl": {
        "welcome": "Hi {user} witaj w Boinkers support bot! Ten bot pomaga w dostępie do portfela, transakcjach, saldach, odzyskiwaniu, odzyskaniu konta, odbieraniu tokenów i nagród, zwrotach i weryfikacji konta. Wybierz opcję, aby kontynuować.",
